...
3. Write concise explanations as `prose`, `formula`, `note`, `steps`, `table`, `figure`, `figurePlaceholder`, and `result` blocks. Put inline TeX inside `\(...\)` and display TeX in `formula.latex`.
   ...
   - Reconstruct mathematical diagrams from the published conditions and calculated coordinates, never by tracing a restricted crop. Prefer generated SVG for graphs, geometry and combinatorial layouts so labels and geometry remain editable. Embed the shipped `KaTeX_Main` and `KaTeX_Math` faces in SVGs loaded through `<img>`; use KaTeX Math italic for variables, KaTeX Main for digits and operators, and the site Japanese sans-serif stack only for Japanese annotations. Keep a text alternative that explains the mathematical relationship rather than merely saying “diagram.”
   - Rasterize every generated SVG at its intended screen size and inspect all labels, not only the newly changed one. No curve, axis, guide, boundary, arrow, or dimension line may pass through a glyph. Move the label into clear space first; use a short leader line when the association would otherwise become unclear, and use an opaque label backing only when relocation cannot preserve the diagram's meaning. Recheck the generated asset after every coordinate adjustment.
