# 日本医科大学 2025年度 数学 — 修正担当への依頼

対象元データ: `C:/---hp/shidai-igakubu-gokaku-dokuhon/projects/universities/nippon-medical/past-exams/working/2025/general-early/first-stage/mathematics`
...
全問監査による追加指摘は [日本医科大学前期数学・正式修復依頼11区分](nippon-early-math-source-review.md) を併せて確認してください。独立SVG5点とHTML増減表を追加しても、元HTMLの論証・時間・解答構造の修復完了ではありません。

修正した正本の位置、原文→修正後、根拠となるページ・計算、再生成したHTML、テスト結果、未解決事項を列挙する。過去問ライブラリーの再取り込み先IDは `nippon-medical-2025-general-early-mathematics`。既存の独自解説へ昇格済みのパッケージは一括取込で上書きしない。
