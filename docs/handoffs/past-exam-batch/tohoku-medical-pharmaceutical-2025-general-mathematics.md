# 東北医科薬科大学 2025年度 数学 — 修正担当への依頼

対象元データ: `C:/---hp/shidai-igakubu-gokaku-dokuhon/projects/universities/tohoku-medical-pharmaceutical/past-exams/working/2025/general/first-stage/mathematics`
...
- **answers / table-column-mismatch**: tohoku-medical-pharmaceutical-2025-general-mathematics-answers-p005 / major-question-03 / f(x)の増減: headers=undefined, rows=6,6,6. Only this table is withheld.
...
修正した正本の位置、原文→修正後、根拠となるページ・計算、再生成したHTML、テスト結果、未解決事項を列挙する。過去問ライブラリーの再取り込み先IDは `tohoku-medical-pharmaceutical-2025-general-mathematics`。既存の独自解説へ昇格済みのパッケージは一括取込で上書きしない。
