# 近畿大学 2025年度 数学 — 修正担当への依頼

2026-09-11の全原本・解説・分析監査で検出した不備は、[正式な修復依頼](kindai-recommendation-math-source-review.md)を優先してください。以下は自動取込の状態記録であり、検出事項の全件一覧ではありません。

対象元データ: `C:/---hp/shidai-igakubu-gokaku-dokuhon/projects/universities/kindai/past-exams/working/2025/recommendation-general-public/first-stage/mathematics`
...
修正した正本の位置、原文→修正後、根拠となるページ・計算、再生成したHTML、テスト結果、未解決事項を列挙する。過去問ライブラリーの再取り込み先IDは `kindai-2025-recommendation-general-public-mathematics`。既存の独自解説へ昇格済みのパッケージは一括取込で上書きしない。
