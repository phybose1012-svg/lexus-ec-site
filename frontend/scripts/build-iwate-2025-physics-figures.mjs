...
add("q1-orbit-circle",650,370,"半径2Rの円軌道と地球。拡大したロケットは左からA、B、Cの順。","円軌道とロケットの配置（模式図）",
  circle(180,210,115,'class="dash"')+circle(180,210,57,'class="glass"')+circle(180,210,2,'fill="#18334c"')+text(160,245,"地球")+dim(180,210,295,210)+text(246,197,"2R")+
  rect(164,91,32,8,'fill="white"')+arrow(198,92,363,64,'class="dash"')+text(423,39,"拡大図")+
  '<path d="M408 64 H550 V108 H408 A22 22 0 0 1 408 64 Z" fill="#edf2f6"/>'+line(490,64,490,108)+line(521,64,521,108)+text(439,93,"A")+text(498,93,"B")+text(529,93,"C")+text(430,140,"3m")+text(496,140,"m")+text(527,140,"m"));

add("q1-orbit-ellipse",640,430,"地球の中心を右側の焦点とするだ円軌道。近地点は中心から2R、遠地点は7R。","だ円軌道の近地点と遠地点",
  '<ellipse cx="310" cy="215" rx="202.5" ry="168.3746" class="dash"/>'+circle(422.5,215,45,'class="glass"')+circle(422.5,215,2,'fill="#18334c"')+dim(107.5,215,422.5,215)+dim(422.5,215,512.5,215)+text(248,200,"7R")+text(470,200,"2R")+text(402,240,"地球")+text(76,250,"遠地点")+text(506,250,"近地点"));
...
function coils() {
  ...
  for(const [x,label,dx] of [[160,"b",-20],[284,"a",12],[450,"d",-20],[574,"c",12]]) body+=circle(x,250,4,'fill="white"')+text(x+dx,255,label);
  return body+text(204,60,"K₁（N₁巻）")+text(489,60,"K₂（N₂巻）")+line(160,254,160,294)+rect(149,294,22,42,'fill="white"')+text(177,322,"R")+arrow(126,335,126,295)+text(105,319,"I");
}
add("q2-core-circuit-switch",760,440,"同じ向きに巻いたK1とK2。K1の左端bに電池の正極と抵抗R、右端aにスイッチと負極を接続。K2の左端dと右端cは開放。Iの正は抵抗を下から上。","スイッチ・電池を接続した回路",
  coils()+line(160,336,160,382)+line(160,382,212,382)+line(212,365,212,399)+line(225,374,225,390)+line(225,382,284,382)+text(200,354,"V")+line(284,382,284,325)+circle(284,325,3,'fill="white"')+line(284,325,304,292)+circle(284,281,3,'fill="white"')+line(284,281,284,254)+text(315,315,"S"));
add("q2-core-circuit-source",760,445,"K1に電源Eと抵抗R、K2のcとdに抵抗rを接続。Iの正はbへ流れ込む向き、iの正は抵抗をcからdへ流れる向き。磁束φの正は鉄心内を右へ。","電源Eと抵抗rを接続した回路・正の向き",
  coils()+line(160,336,160,382)+line(160,382,202,382)+rect(202,363,52,38,'fill="white"')+text(220,389,"E")+line(254,382,284,382)+line(284,382,284,254)+line(450,254,450,330)+line(450,330,494,330)+rect(494,319,56,22,'fill="white"')+text(518,308,"r")+line(550,330,574,330)+line(574,330,574,254)+arrow(551,366,494,366)+text(520,394,"i")+arrow(342,127,411,127)+text(369,114,"φ"));

add("q2-current-graph",640,345,"電流iは0からTまで−i0、Tから3Tまで0、3Tから4Tまでi0、4Tより後は0。","電流iの時間変化",
  arrow(44,175,589,175)+arrow(70,305,70,28)+text(601,182,"t")+text(54,20,"i")+text(46,195,"0")+
  ...
  line(70,270,160,270,'stroke-width="3"')+line(160,175,340,175,'stroke-width="3"')+line(340,80,430,80,'stroke-width="3"')+line(430,175,560,175,'stroke-width="3"')+text(35,86,"i₀")+text(21,277,"−i₀")+text(153,203,"T")+text(327,203,"3T")+text(417,203,"4T"));
...
add("ans-q3-lens-geometry",640,470,"共通の接点Pから測る球面の高さはd1とd2。各球面の中心O1、O2から半径R1、R2を引き、横の長さr、縦の長さRj−djの直角三角形を作る。空隙dはd2−d1。","球面の高さd₁・d₂と、空隙d = d₂ − d₁",
  ...
  circle(x0,y0-R1,3,'fill="#18334c"')+circle(x0,y0-R2,3,'fill="#18334c"')+text(195,y0-R1+7,"O₁")+text(195,y0-R2+7,"O₂")+
  line(x0,y0-R1,x0+r,y0-d1)+line(x0,y0-R2,x0+r,y0-d2)+text(328,175,"R₁")+text(312,237,"R₂")+
  line(x0,y0-d1,x0+r,y0-d1,'class="dash"')+line(x0,y0-d2,x0+r,y0-d2,'class="dash"')+line(x0+r,y0-d2,x0+r,y0,'class="dash"')+
  dim(x0,y0+31,x0+r,y0+31)+text(x0+r/2-5,y0+58,"r")+text(x0-24,y0+25,"P")+
  line(x0+r,y0-d1,570,y0-d1,'class="dash"')+line(x0+r,y0-d2,570,y0-d2,'class="dash"')+
  dim(455,y0-d2,455,y0-d1)+text(464,y0-(d1+d2)/2+6,"d")+dim(514,y0-d1,514,y0)+text(523,y0-d1/2+6,"d₁")+dim(581,y0-d2,581,y0)+text(590,y0-d2/2+6,"d₂")+
  text(74,267,"R₁ − d₁",'class="small"')+text(244,266,"R₂ − d₂",'class="small"'));
